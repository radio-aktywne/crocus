...
import {
  safeAcceptLogoutRequest,
  safeDecryptAuthLogoutAcceptResponse,
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const { challenge } = await safeDecryptAuthLogoutAcceptResponse({
    data: token,
  });
  ...
  const { redirect: url } = await safeAcceptLogoutRequest({
